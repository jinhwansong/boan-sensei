...
| Claude Code skill adapter (`SKILL.md`) | 확인 안 됨 (근거: local Claude Code skill runtime not available in this environment) | `adapters/claude/boan-sensei/SKILL.md` and `plugins/claude-code-boan-sensei/boan-sensei/SKILL.md` are structured as Skill documents rather than slash commands or Cursor-style rules. Local Claude Code automatic skill trigger behavior was not executed in this environment. | Treat Claude Code support as a Skill-based review workflow candidate until a local Claude Code session confirms that the skill is discovered and triggered as intended. |
...
- Claude Code: do not describe the Skill document as a slash command or Cursor rule. It is a Skill-based workflow candidate, and automatic trigger behavior still needs local verification.

...
import type { Finding, FindingsFile, ScanProjectOptions } from "./types.js";
...
export async function scanProject(projectRoot = process.cwd(), options: ScanProjectOptions = {}): Promise<Finding[]> {
  ...
  if (options.write) {
    await writeFindingsFile(root, findings);
  }

  return findings;
}

export async function writeFindingsFile(projectRoot: string, findings: Finding[]): Promise<FindingsFile> {
